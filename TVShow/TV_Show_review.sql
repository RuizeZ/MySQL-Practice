...
SELECT title, round(avg(rating), 2) as avg_rating FROM series JOIN reviews ON series.id = reviews.series_id GROUP BY series.title ORDER BY avg_rating;
...
SELECT first_name, last_name, count(rating) as count, ifnull(min(rating), 0) as min, ifnull(max(rating), 0) as max, ifnull(round(avg(rating), 2), 0) as avg_rating,
CASE
